...
bool SortCoords::compareX(Vertex V1, Vertex V2)
{
...
bool SortCoords::compareY(Vertex V1, Vertex V2)
{
	if (V1.m_xyz[1] == V2.m_xyz[1])
	{
		if (V1.m_xyz[0] < V2.m_xyz[0])
...
void SortCoords::SortX(std::vector<Vertex> &points)
{
	std::sort(points.begin(), points.end(), compareX);
}

void SortCoords::SortY(std::vector<Vertex>& points)
{
	std::sort(points.begin(), points.end(), compareY);
}

float SortCoords::FindXMin(std::vector<Vertex>& points)
{
	if (points.size() < 1)
		return -1.f;
...
float SortCoords::FindYMin(std::vector<Vertex>& points)
{
	if (points.size() < 1)
		return -1.f;
...
float SortCoords::FindXMax(std::vector<Vertex>& points)
{
	if (points.size() < 1)
		return -1.f;
...
float SortCoords::FindYMax(std::vector<Vertex>& points)
{
	if (points.size() < 1)
		return -1.f;
