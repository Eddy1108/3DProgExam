...
Equidistance* LAZSurface::constructEquidistance()
{
    if(mEquiLines)
    {
        std::cout << "Error! Equidistance has already been constructed! A second attempt to construct it has been made" << std::endl;
        return mEquiLines;
    }
    ...
    for(int i = 0; i < mIndices.size(); i+=3){
        ...
        if(maxHeight >= minHeight + mEquidistance - fmod(minHeight, mEquidistance))
        {
            ...
            //First point
            direction = abc[2]->getPos() - abc[0]->getPos();
            direction.normalize();
            float multiplier = (checkHeight - minHeight)/direction.z();
            p0 = abc[0]->getPos() + (direction * multiplier);
            // Second point
            if(checkHeight <= midHeight) // Second point is on line AB
            {
                direction = abc[1]->getPos() - abc[0]->getPos();
                direction.normalize();
                float multiplier = (checkHeight - minHeight)/direction.z();
                p1 = abc[0]->getPos() + (direction * multiplier);
            }
            else // Second point is on line BC
            {
                direction = abc[2]->getPos() - abc[1]->getPos();
                direction.normalize();
                float multiplier = (checkHeight - midHeight)/direction.z();
                p1 = abc[1]->getPos() + (direction * multiplier);
            }
            ...
            while(checkHeight + mEquidistance <= maxHeight){
                ...
                if(maxHeight == minHeight && checkHeight + mEquidistance == maxHeight){
                    mEquiLines->insertLine(abc[1]->getPos(), abc[2]->getPos());
                }
